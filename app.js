...
function renderProducts(list) { 
    ...
    grid.innerHTML = list.map(function(p) {
        var isUrl = p.image && p.image.startsWith('http');
        var imageHTML = isUrl ? '<img src="' + p.image + '" alt="' + p.name + '" style="max-width:100%; max-height:100%; object-fit:contain;">' : p.image;
        ...
        return '<div class="product-card ' + (outOfStock ? 'out-of-stock' : '') + '">' +
            ...
            '<a href="product.html?product=' + p.slug + '" style="text-decoration:none;"><div class="product-image">' + imageHTML + '</div></a>' +
            '<div class="product-info">' +
            '<a href="product.html?product=' + p.slug + '" style="text-decoration:none; color:inherit;"><h3 class="product-title">' + p.name + '</h3>' +
            (p.nameAr ? '<p class="product-title-ar">' + p.nameAr + '</p>' : '') + '</a>' +
            '<div class="product-price">AED ' + p.price + '</div>' +
            (outOfStock ? '<button class="add-to-cart" disabled style="background:#999;cursor:not-allowed;">Out of Stock | نفذ المخزون</button>' : '<button class="add-to-cart" onclick="addToCart(' + p.id + ', event)">Add to Cart | أضف إلى السلة</button>') +
            '</div></div>';
    }).join(""); 
}
...
function loadProducts(category) { 
    ...
    var list = category === "All Products" ? products : products.filter(function(p) { return p.category === category; }); 
    renderProducts(list); 
...
function createCategoryFilters() { 
    ...
    container.innerHTML = getCategories().map(function(cat) {
        var catAr = getCategoryArabic(cat);
...
function updateCategoryButtons() { 
    document.querySelectorAll(".category-btn").forEach(function(btn) { 
        var firstLine = btn.childNodes[0]; 
        if (firstLine && firstLine.textContent) { 
            var catText = firstLine.textContent.trim(); 
...
function searchProducts() { 
    ...
    var scoped = selectedCategory === "All Products" ? products : products.filter(function(p) { return p.category === selectedCategory; }); 
    var results = scoped.filter(function(p) { return p.name.toLowerCase().includes(term) || p.description.toLowerCase().includes(term) || p.category.toLowerCase().includes(term); }); 
...
function addToCart(id, event) { 
    ...
    var currentInCart = item ? item.quantity : 0;
    var maxAllowed = Math.min(MAX_QTY_PER_PRODUCT, product.quantity);
    if (currentInCart >= maxAllowed) return;
    if (item) { 
        item.quantity++; 
    } else { 
        var newItem = {};
        for (var key in product) { newItem[key] = product[key]; }
        newItem.quantity = 1;
        cart.push(newItem); 
    } 
    saveCart(); 
...
function showCartPopup(product) {
    ...
    var cartCountVal = cart.reduce(function(sum, item) { return sum + item.quantity; }, 0);
    var isUrl = product.image && product.image.startsWith('http');
    var imageHTML = isUrl ? '<img src="' + product.image + '" alt="' + product.name + '" style="width:100%; height:100%; object-fit:contain;">' : '<span style="font-size:3rem;">' + (product.image || '📦') + '</span>';
    var popup = document.getElementById('cartPopup');
    var popupContent = document.getElementById('cartPopupContent');
    if (!popup || !popupContent) return;
    popupContent.innerHTML = '<div class="popup-top"><button class="popup-close-btn" onclick="closeCartPopup()">✕</button><div class="popup-success-badge">✓ Success!</div><div class="popup-title">Added to Cart</div><div class="popup-title-ar">تمت الإضافة للسلة</div></div><div class="popup-product-float"><div class="popup-product-card"><div class="popup-product-image">' + imageHTML + '</div><div class="popup-product-details"><div class="popup-product-name">' + product.name + '</div>' + (product.nameAr ? '<div class="popup-product-name-ar">' + product.nameAr + '</div>' : '') + '<div class="popup-product-price">AED ' + product.price + '</div></div></div></div><div class="popup-bottom"><div class="popup-cart-summary"><span class="popup-summary-label">Cart Total (' + cartCountVal + ' ' + (cartCountVal === 1 ? 'item' : 'items') + '):</span><span class="popup-summary-value">AED ' + cartTotal.toFixed(2) + '</span></div><div class="popup-buttons"><button class="popup-btn-view-cart" onclick="closeCartPopup(); toggleCart();">🛒 View Cart | <span class="arabic-text">عرض السلة</span></button><button class="popup-btn-continue" onclick="closeCartPopup()">Continue Shopping | <span class="arabic-text">متابعة التسوق</span></button></div></div>';
...
function updateCart() {
    ...
    var bottomCartCount = document.getElementById("bottomCartCount");
    var mobileCartCount = document.getElementById("mobileCartCount");
    var cartFooter = document.querySelector(".cart-footer");
    var cartCheckoutFixed = document.getElementById("cartCheckoutFixed");
    ...
    var checkoutBtnHTML = '<button id="stripeBtn" style="width: 100%; padding: 0.9rem; font-size: 0.95rem; font-weight: 600; border: none; border-radius: 8px; cursor: pointer; background: #2c4a5c; color: white; transition: all 0.3s;" onclick="checkout()" onmouseover="this.style.background=\'#1e3545\'" onmouseout="this.style.background=\'#2c4a5c\'">💳 Pay with Card / الدفع بالبطاقة</button>';
    ...
    cartItems.innerHTML = cart.map(function(i) { return '<div style="display:flex; justify-content:space-between; align-items:center; padding:0.5rem; border-bottom:1px solid #eee;"><div style="flex:1;"><strong style="font-size:0.9rem; color:#2c4a5c;">' + i.name + '</strong><br><span style="color:#888; font-size:0.8rem;">AED ' + i.price + ' × ' + i.quantity + '</span><br><span style="color:#e07856; font-weight:600; font-size:0.9rem;">AED ' + (i.price * i.quantity).toFixed(2) + '</span></div><div style="display:flex; gap:0.4rem; align-items:center;"><button onclick="updateQuantity(' + i.id + ', -1)" style="padding:0.3rem 0.6rem; background:#f0f0f0; border:none; border-radius:4px; cursor:pointer; font-size:0.85rem; font-weight:600;">-</button><span style="font-size:0.9rem; font-weight:600; min-width:20px; text-align:center;">' + i.quantity + '</span><button onclick="updateQuantity(' + i.id + ', 1)" style="padding:0.3rem 0.6rem; background:#f0f0f0; border:none; border-radius:4px; cursor:pointer; font-size:0.85rem; font-weight:600;">+</button><button onclick="removeFromCart(' + i.id + ')" style="padding:0.3rem 0.6rem; background:#dc3545; color:white; border:none; border-radius:4px; cursor:pointer; margin-left:0.3rem; font-size:0.85rem;">✕</button></div></div>'; }).join(""); 
    var footerHTML = '';
    var amountNeededForFree = FREE_DELIVERY_THRESHOLD - subtotal;
    var showUpsell = subtotal < FREE_DELIVERY_THRESHOLD && !(isMobile && upsellUsed);
    if (showUpsell) {
        var cartProductIds = cart.map(function(i) { return i.id; });
        var upsellProducts = products.filter(function(p) { return !cartProductIds.includes(p.id); }).filter(function(p) { return p.quantity > 0; }).filter(function(p) { return p.price >= amountNeededForFree; }).sort(function(a, b) { return a.price - b.price; }).slice(0, 2);
        if (subtotal >= 60 && upsellProducts.length > 0) {
            footerHTML += '<div style="padding: 0.75rem 1rem; background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; margin-bottom: 0.75rem;"><div style="font-weight: 600; margin-bottom: 0.75rem; color: #2c4a5c; font-size: 0.9rem;">Add AED ' + amountNeededForFree.toFixed(0) + ' more for free delivery:</div>' + upsellProducts.map(function(p) { return '<div style="display: flex; align-items: center; padding: 0.25rem 0; border-bottom: 1px solid #f0f0f0; gap: 0.5rem;"><div style="flex: 1; font-weight: 500; color: #2c4a5c; font-size: 0.8rem;">' + p.name + '</div><div style="font-size: 0.75rem; color: #888; white-space: nowrap;">AED ' + p.price + '</div><button onclick="addUpsellItem(' + p.id + ', event)" style="padding: 0.25rem 0.5rem; background: #2c4a5c; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.75rem;">Add</button></div>'; }).join('') + '</div>';
        } else if (subtotal < 60) {
            footerHTML += '<div style="padding: 0.75rem 1rem; background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; margin-bottom: 0.75rem;"><div style="font-weight: 600; color: #2c4a5c; font-size: 0.9rem; margin-bottom: 0.5rem;">🚚 Add AED ' + amountNeededForFree.toFixed(0) + ' more to qualify for free delivery</div></div>';
        }
    }
    if (subtotal >= FREE_DELIVERY_THRESHOLD) { savedUpsellProducts = null; }
    footerHTML += '<div style="padding: 1rem; background: #f8f9fa; border-radius: 8px; margin-bottom: 0.75rem;"><div style="display: flex; justify-content: space-between; padding: 0.5rem 0; font-size: 0.9rem; color: #2c4a5c;"><span>Subtotal / المجموع الفرعي:</span><span>AED ' + subtotal.toFixed(2) + '</span></div><div style="display: flex; justify-content: space-between; padding: 0.5rem 0; font-size: 0.9rem; color: #2c4a5c;"><span>Delivery / التوصيل:</span><span style="' + (deliveryFee === 0 ? 'color: #28a745; font-weight: 600;' : '') + '">' + (deliveryFee === 0 ? 'FREE / مجاني' : 'AED ' + deliveryFee.toFixed(2)) + '</span></div><div style="border-top: 2px solid #ddd; margin: 0.5rem 0;"></div><div style="display: flex; justify-content: space-between; padding: 0.75rem 0 0.5rem; font-size: 1.1rem; font-weight: 700; color: #2c4a5c;"><span>Total / الإجمالي:</span><span>AED ' + total.toFixed(2) + '</span></div></div>';
    if (!isMobile) { footerHTML += '<div style="padding: 0 1rem 1rem;">' + checkoutBtnHTML + '</div>'; }
...
function updateQuantity(id, change) { 
    ...
    if (item) { 
        ...
        if (change > 0) {
            var maxAllowed = Math.min(MAX_QTY_PER_PRODUCT, product ? product.quantity : MAX_QTY_PER_PRODUCT);
            if (newQty > maxAllowed) return;
        }
        item.quantity = newQty;
        if (item.quantity <= 0) { removeFromCart(id); } else { saveCart(); updateCart(); } 
    } 
}

function removeFromCart(id) { cart = cart.filter(function(i) { return i.id !== id; }); upsellUsed = false; saveCart(); updateCart(); }
...
function toggleCart() { 
    ...
    if (cartSidebar.classList.contains("active")) {
        ...
        upsellUsed = false; savedUpsellProducts = null;
    }
    updateCart();
}

function addUpsellItem(id, event) { if (window.innerWidth <= 768) upsellUsed = true; addToCart(id, event); }

function openPolicy(type) { 
    var policyText = document.getElementById("policyText");
    var policyModal = document.getElementById("policyModal");
    if (policyText) policyText.innerHTML = policies[type];
    if (policyModal) { policyModal.style.display = "block"; document.body.style.overflow = "hidden"; }
}

function closePolicy() { 
    var policyModal = document.getElementById("policyModal");
    if (policyModal) { policyModal.style.display = "none"; document.body.style.overflow = "auto"; }
}

function toggleAbout() {
    var aboutSection = document.getElementById('about');
    if (!aboutSection) return;
    var isVisible = window.getComputedStyle(aboutSection).display !== 'none';
    if (isVisible) { aboutSection.style.display = 'none'; } else { aboutSection.style.display = 'block'; aboutSection.scrollIntoView({ behavior: 'smooth' }); }
}
...
function toggleMobileMenu() {
    ...
    if (!overlay) {
        ...
        overlay.innerHTML = '<div class="mobile-menu"><a href="#products" onclick="closeMobileMenu()"><span class="menu-en">🛍️ Shop</span> | <span class="menu-ar">تسوق</span></a><a href="#about" onclick="closeMobileMenu()"><span class="menu-en">ℹ️ About</span> | <span class="menu-ar">من نحن</span></a><a href="#contact" onclick="closeMobileMenu()"><span class="menu-en">📧 Contact</span> | <span class="menu-ar">اتصل بنا</span></a><a href="#terms" onclick="closeMobileMenu()"><span class="menu-en">📋 Terms</span> | <span class="menu-ar">الشروط</span></a></div>';
        document.body.appendChild(overlay);
        overlay.onclick = function(e) { if (e.target === overlay) closeMobileMenu(); };
    }
    overlay.classList.toggle('active');
}

function closeMobileMenu() { var overlay = document.querySelector('.mobile-menu-overlay'); if (overlay) overlay.classList.remove('active'); }

window.onload = function() { 
    if (document.getElementById("categoryFilters")) { createCategoryFilters(); loadProducts(); }
    updateCart();
    var urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('showAbout') === 'true') { var aboutSection = document.getElementById('about'); if (aboutSection) { aboutSection.style.display = 'block'; setTimeout(function() { aboutSection.scrollIntoView({ behavior: 'smooth' }); }, 100); } }
    var searchTerm = urlParams.get('search');
    if (searchTerm) { var searchInput = document.getElementById('searchInput'); if (searchInput) { searchInput.value = searchTerm; searchProducts(); } }
    var promoBanner = document.querySelector('.mobile-promo-banner');
    if (promoBanner) promoBanner.innerHTML = '🚚 Free delivery over AED ' + FREE_DELIVERY_THRESHOLD + ' | <span class="arabic-text">توصيل مجاني فوق ' + FREE_DELIVERY_THRESHOLD + ' درهم</span>';
    var heroThreshold = document.getElementById('heroThreshold');
    var heroThresholdAr = document.getElementById('heroThresholdAr');
    ...
    if (searchInputEl) searchInputEl.onkeypress = function(e) { if (e.key === "Enter") { e.preventDefault(); searchProducts(); } };
    if (cartIcon) cartIcon.onclick = toggleCart;
    if (closeCartBtn) closeCartBtn.onclick = toggleCart;
    if (policyModal) policyModal.onclick = function(e) { if (e.target.id === "policyModal") closePolicy(); };
    var bottomHomeBtn = document.getElementById("bottomHomeBtn");
    var bottomCartBtn = document.getElementById("bottomCartBtn");
    ...
    if (bottomHomeBtn) {
        ...
        bottomHomeBtn.onclick = function() {
            var cartSidebar = document.getElementById("cartSidebar");
            if (cartSidebar && cartSidebar.classList.contains("active")) { cartSidebar.classList.remove("active"); if (bottomCartBtn) bottomCartBtn.classList.remove("cart-active"); upsellUsed = false; savedUpsellProducts = null; }
            closeMobileMenu(); bottomHomeBtn.classList.add("home-active"); window.scrollTo({top: 0, behavior: 'smooth'});
        };
    }
...
async function checkout() {
    ...
    var originalText = btn ? btn.innerHTML : "Pay with Card";
    try {
        if (btn) { btn.disabled = true; btn.innerHTML = "Checking stock..."; }
        var response = await fetch('/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cart: cart, deliveryZoneKey: selectedDeliveryZone }) });
        var data = await response.json();
        if (data.error) {
            if (data.error === 'out_of_stock') { alert(data.message); if (typeof initProducts === 'function') initProducts(); }
            else if (data.error === 'insufficient_stock') { var msg = 'Stock issue:\n'; data.items.forEach(function(item) { msg += item.name + ': Only ' + item.available + ' available (you wanted ' + item.requested + ')\n'; }); alert(msg); }
            else { alert(data.message || 'Payment failed. Please try again.'); }
            if (btn) { btn.disabled = false; btn.innerHTML = originalText; }
            return;
        }
        if (data.url) { window.location.href = data.url; } else { throw new Error('No URL'); }
    } catch (err) {
